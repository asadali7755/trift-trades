...
function MagneticPill({
  ...
  const content = (
    <motion.div
      ...
      className={
        primary
          ? "inline-flex items-center gap-2 rounded-full bg-accent px-8 py-4 text-xs font-bold uppercase tracking-widest text-ink transition hover:bg-accent-dark"
          : "inline-flex items-center gap-2 rounded-full border border-white/15 px-5 py-2.5 text-xs font-medium uppercase tracking-widest text-paper/70 backdrop-blur transition hover:border-accent/50 hover:text-accent"
      }
    >
      {children}
    </motion.div>
